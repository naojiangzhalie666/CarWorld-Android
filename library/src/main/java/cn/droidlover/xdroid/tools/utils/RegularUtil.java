...
public class RegularUtil {
    ...
    // 数字字母组合
    private static Pattern numericAbcPattern = Pattern.compile("^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{8,16}$");
    // float
     ...
     *         源字符串
     */
    public static boolean isNumericABC(String src) {
        boolean return_value = false;
        if (src != null && src.length() > 0) {
            Matcher m = numericAbcPattern.matcher(src);
            if (m.find()) {
                return_value = true;
            }
        }
