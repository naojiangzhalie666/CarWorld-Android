...
public class LineGraphicView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        if (xRawDatas == null || yRawData == null) {
            return;
        }
        mPaint.setColor(res.getColor(R.color.main_bg));
        // 画直线（纵向）
        drawAllYLine(canvas);
        drawAllXLine(canvas);
        // 点的操作设置
        ...
        mPaint.setColor(res.getColor(R.color.colorDarkBlue));
        ...
        for (int i = 0; i < mPoints.length; i++) {
            if (i != 0) {
                ...
                canvas.drawText(String.valueOf(yRawData.get(i)), mPoints[i].x - 10, mPoints[i].y - 25, mPaint);
            }
        }
    }
    ...
    private void drawAllYLine(Canvas canvas) {
        for (int i = 0; i < yRawData.size(); i++) {
            ...
            canvas.drawLine(blwidh + (canvasWidth - blwidh) / yRawData.size() * i, marginTop,
                    blwidh + (canvasWidth - blwidh) / yRawData.size() * i,
                    bheight + marginTop, mPaint);

        }
        for (int i = 0; i < xRawDatas.size(); i++) {
            drawText(xRawDatas.get(i), (blwidh + (canvasWidth - blwidh) / yRawData.size() * i) - 20,
                    bheight + dip2px(26), canvas);// X坐标
        }
    }
    ...
    private void drawText(String text, int x, int y, Canvas canvas) {
        ...
        p.setColor(res.getColor(R.color.color_999));
    ...
    private Point[] getPoints() {
        ...
        for (int i = 0; i < yRawData.size(); i++) {
            int ph = bheight - (int) (bheight * (Double.valueOf(yRawData.get(i)) / maxValue));
    ...
    public void setData(ArrayList<String> yRawData, ArrayList<String> xRawData, int maxValue, int averageValue) {
